class OrthologGroupMembersInFixedTaxonomyTreeStanza < TogoStanza::Stanza::Base
  ...
  org = query('http://mbgd.genome.ad.jp:7050/sparql', <<-SPARQL.strip_heredoc)
	...
	PREFIX orthology: <http://purl.org/net/orth#>
        SELECT ?taxon (COUNT(?organism) AS ?organism_count)
         WHERE {
		   ...
		   orthology:hasHomologous/mbgd:organism ?organism .
  ...
  member = query('http://mbgd.genome.ad.jp:7050/sparql', <<-SPARQL.strip_heredoc)
      PREFIX dct: <http://purl.org/dc/terms/>
      PREFIX void: <http://rdfs.org/ns/void#>
      PREFIX mbgd: <http://purl.jp/bio/11/mbgd#>
      PREFIX mbgdr: <http://mbgd.genome.ad.jp/rdf/resource/>
   ...
   all = query('http://mbgd.genome.ad.jp:7050/sparql', <<-SPARQL.strip_heredoc)
        PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
        PREFIX mbgd: <http://purl.jp/bio/11/mbgd#>
        PREFIX mbgdr: <http://mbgd.genome.ad.jp/rdf/resource/>
        ...
        WHERE {
         mbgdr:#{dataset} mbgd:organism ?organism .
...
root = {:tax => "http://identifiers.org/taxonomy/131567", :cnt => 1, :tax_label => "Cellular_organism", :ratio => 1, :rank => "http://http://ddbj.nig.ac.jp/ontologies/taxonomy/Superkingdom", :parent => "root", :link => "http://mbgd.genome.ad.jp"}
   all.push(root)
